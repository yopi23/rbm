...
class TgDataUpdateSeeder extends Seeder
{
    public function run()
    {
        // SEEDER AMAN PRODUKSI - TANPA TRUNCATE / HAPUS DATA
        ...
        foreach ($dataGroups as $group) {
            ...
            foreach ($models as $modelName) {
                ...
                $hp = HpData::where('type', $modelName)->first();
                if ($hp) {
                    $hp->update([
                        'code_tg' => $kode,
                        'brand_id' => $hp->brand_id ?: $brand->id,
                        'screen_size_id' => $hp->screen_size_id ?: $screenSize->id,
                        'camera_position_id' => $hp->camera_position_id ?: $cameraPos->id,
                    ]);
                } else {
                    HpData::create([
                        'type' => $modelName,
