    ...
    public function storePengeluaranOperasional(Request $request): JsonResponse
{
    ...
    public function getSummary(Request $request): JsonResponse
    {
        try {
            ...
            if ($startDate && $endDate) {
                Log::debug('Menambahkan filter tanggal', [
                    'start_date' => $startDate,
                    'end_date' => $endDate
                ]);

                $tokoQuery->whereBetween('tanggal_pengeluaran', [$startDate, $endDate]);
                $opexQuery->whereBetween('tgl_pengeluaran', [$startDate, $endDate]);
            }
    ...
    /**
     * Get kategori laci for dropdown
     */
    public function getKategoriLaci(): JsonResponse
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Data kategori laci berhasil diambil',
                'data' => $kategoriLaci
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data kategori laci',
                'error' => $e->getMessage()
            ], 500);
        }
    }
